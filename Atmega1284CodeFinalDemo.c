...
#define DD_MISO PB6
#define DD_SCK PB7
unsigned char start_system = 0;
unsigned char detection_status = 0;
...
//void SPI_Transmit(unsigned char myVar) {
//	SPDR = start_system;
//	while(!(SPSR & (1<<SPIF))){}
//}
...
enum LCD_States {init_lcd, lcd_display, lcd_wait, lcd_password, lcd_arm, lcd_armed, lcd_intrustion, lcd_unarm, lcd_unarmed, lcd_alert} lcd_state;
...
enum SPI_States {spi_init, spi_transmit, spi_recieve} spi_state;
...
void LEDFct() {
	...
	switch(led_state) {
		case led_display:
		if(alarm_switch && (system_intrusion && lcd_state != lcd_arm)) {
		column_sel = 0x00;
		column_val = 0xFF;
		}
		else {
...
void LCDFct() {
	...
	switch(lcd_state) {
		...
		break;
		case lcd_alert:
		LCD_DisplayString(1, "Alert -- Calling Police");
		break;
	}
...
void SPIFct() {
	...
	switch(spi_state) {
		...
		case spi_transmit:
		spi_state = spi_transmit;
		if(keypad_state == keypad_wait || keypad_state == keypad_wait_again) {
			spi_state = spi_recieve;
			detection_status = SPDR;
		}
		break;
		case spi_recieve:
...
int main(void)
{
	...
	unsigned long int SPIFct_Period = 500;
	...
	// Task 4
	task4.state = spi_init;//Task initial state.
	task4.period = SPIFct_Period;//Task Period.
	task4.elapsedTime = SPIFct_Period;//Task current elapsed time.
	task4.TickFct = &SPIFct;//Function pointer for the tick.
