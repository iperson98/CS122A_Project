...
enum LCD_States {init_lcd, lcd_display, lcd_wait, lcd_password, lcd_arm, lcd_armed} lcd_state;
enum Keypad_States {keypad_enter, keypad_password, keypad_compare, keypad_wait} keypad_state;
...
void LCDFct() {
	...
	switch(lcd_state) {
		...
		break;
		case lcd_armed:
		LCD_DisplayString(1, "System Armed");
		break;
	}
}
